...
export default function NewExerciseForm() {
    ...
    const handleSubmit = e => {
        e.preventDefault()
        const data = {user_id:Number(userId), description, duration:Number(duration), date}
        if (data.user_id && data.description && data.duration) {
            // create new Exercise
            const API_URL = process.env.NODE_ENV === "development" ? "http://localhost:4747/api/exercise/add" : "/api/exercise/add"
            fetch(API_URL, {
                method:"POST",
    ...
    return(
        <form className="mt-4 bg-gray-100 p-2 rounded md:flex-grow md:ml-4" onSubmit={handleSubmit}>
            ...
            <div className="flex mt-2 items-center">
                <label htmlFor="description" className="flex-shrink-0">Description*:</label>
                <input type="text" required id="description" name="description" className="p-1 rounded ml-2 w-full" onChange={handleDescriptionChange} value={description}/>
            </div>
            <div className="flex mt-2 items-center">
                <label htmlFor="duration" className="flex-shrink-0">Duration*:</label>
                <input type="number" required id="duration" name="duration" className="p-1 rounded ml-2 w-full" onChange={handleDurationChange} value={duration} placeholder="mins"/>
            </div>
            <div className="flex mt-2 items-center">
